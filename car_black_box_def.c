 ...
 *
 * Created on 1 August, 2024, 1:00 PM
 */
...
unsigned char long_press_detected = 0;
...
unsigned char login_menu(unsigned char key, unsigned char reset_flag)
{
    ...
    if (key_pressed && !long_press_detected)
    {
        if (key_pressed == SW4 && menu_pos < 4) /* increment menu position on short press */
        {
            menu_pos++; 
            clear_screen();
        }
        else if (key_pressed == SW5 && menu_pos > 0) /* decrement menu position on short press */
        {
            menu_pos--; 
            clear_screen();
        }
